...
async def remove_test(uuid_obj, test_type: str):
    ...
    test, _ = await prepare_test_for_action(uuid_obj, test_type, action="delete")
    ...
    return success_response({"message": gettext(f"{test_type.capitalize()} test deleted successfully")})
