...
async def get_course_enrollment_status_cache_key(course_id, user_id) -> str:
    key = f"course_enrollment_status_by_id_{course_id}:{user_id}"
...
async def get_course_enrollment_status_cache(course_id, user_id):
    ...
    cache_key = await get_course_enrollment_status_cache_key(course_id, user_id)
...
async def invalidate_course_enrollment_status_cache(course_id, user_id) -> bool:
    cache = caches["courses_get"]
    cache_key = await get_course_enrollment_status_cache_key(course_id, user_id)
