...
async def remove_course_from_wishlist(user_id, course_id):
    user_id = validate_uuid(user_id)
    course_id = validate_uuid(course_id)

    deleted_course = await sync_to_async(Wishlist.objects.filter(course_id=course_id, user_id=user_id).delete)()
