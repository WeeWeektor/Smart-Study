...
def success_response(data=None, message="Успішно"):
...
def send_template_email(user, subject, url_path, html_template_func, plain_template_func):
    ...
    if not token:
        raise ValueError("Не вдалося створити токен активації.")
    base_url = getattr(settings, 'BASE_URL', 'https://127.0.0.1:8000')
    full_url = f"{base_url}{url_path}?token={token}"
    greeting = f"Вітаємо, {user.name}!" if user.name else "Вітаємо!"
...
def send_verification_email(user):
    send_template_email(
        user=user,
        subject="Підтвердження пошти для SmartStudy",
...
def send_password_reset_email(user):
    send_template_email(
        user=user,
        subject="Відновлення пароля для SmartStudy",
