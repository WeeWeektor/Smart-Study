...
export const SocialAuth = ({
  ...
  const {
    ...
    handleGoogleClick,
    handleFacebookClick,
  } = useSocialAuth({ onError })

  if (showExtraFields) {
    return (
      <form onSubmit={handleExtraFieldsSubmit} className="space-y-3">
        <div>
          <label>Прізвище</label>
          <input
            type="text"
            value={extraFields.surname}
            onChange={e => handleExtraFieldsChange('surname', e.target.value)}
            required
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={e => handleExtraFieldsChange('name', e.target.value)}
            required
          />
        </div>
        <div>
          ...
          <select
            ...
            onChange={e => handleExtraFieldsChange('role', e.target.value)}
            required
          >
        ...
        <div>
          ...
          <input
            type="text"
            value={extraFields.phone_number}
            onChange={e =>
              handleExtraFieldsChange('phone_number', e.target.value)
            }
          />
        </div>
  ...
  return (
    <div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3">
        <button
          type="button"
          className="border border-slate-300 text-slate-700 rounded-md px-4 py-2 flex items-center justify-center w-full"
